...
public class SudoSolver {
    ...
    private int[] getPossible(int[][] field, int y, int x) {
        ...
        for (int i = 0; i < field.length; i++) {
            poss.remove(Integer.valueOf(Math.abs(field[x][i])));
            poss.remove(Integer.valueOf(Math.abs(field[i][y])));
        }
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                poss.remove(Integer.valueOf(Math.abs(field[i + quadStartX][j + quadStartY])));
            }
        }
